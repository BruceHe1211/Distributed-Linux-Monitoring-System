...
QWidget *MonitorWidget::ShowAllMonitorWidget(const std::string& name) {
  ...
  stack_menu_->addWidget(InitProcessMonitorWidget());
...
QWidget *MonitorWidget::InitButtonMenu(const std::string& name) {
  ...
  QPushButton *process_button = new QPushButton(QString::fromStdString(name) + "_process", this);
  ...
  connect(process_button, SIGNAL(clicked()), this, SLOT(ClickProcessButton()));
...
//新增process窗口
QWidget *MonitorWidget::InitProcessMonitorWidget() {
  QWidget *widget = new QWidget();

  QLabel *process_label = new QLabel(this);
  process_label->setText(tr("Monitor process:"));
  process_label->setFont(QFont("Microsoft YaHei", 10, 40));

  process_monitor_view_ = new QTableView;
  process_model_ = new ProcessModel;
  process_monitor_view_->setModel(process_model_);
  process_monitor_view_->show();

  QGridLayout *layout = new QGridLayout();

  layout->addWidget(process_label, 1, 0);
  layout->addWidget(process_monitor_view_, 2, 0, 1, 1);
...
void MonitorWidget::UpdateData(
  ...
  process_model_->UpdateMonitorInfo(monitor_info); // 新增更新数据
  std::cerr << "try to run UpdateData in monitor_widget.cpp" << std::endl;
}
...
void MonitorWidget::ClickProcessButton() { stack_menu_->setCurrentIndex(4); } // 新增槽
}  // namespace monitor
